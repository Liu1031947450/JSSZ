...
export default function Admin() {
  ...
  useEffect(() => { if (!draft?.editing) editorTrigger.current?.focus({ preventScroll: true }); }, [Boolean(draft?.editing)]);
  ...
  const saveNotice = saveError && <div className="notice error" role="alert"><span>{saveError}</span><Button size="small" disabled={busy || !draft} onClick={() => { if (draft) void persist(draft).catch(() => {}); }}>重试保存</Button></div>;
  const syncNotification = syncNotice && <div className={`animal-notification-root${editing ? ' editor-notification' : ''}`} role="status" aria-live="polite" aria-atomic="true"><div className="animal-notification-group animal-notification-group-top"><NotificationView key={syncNotice.key} item={syncNotice} onRemove={() => setSyncNotice(null)} /></div></div>;

  return <div className="admin-page">
    {!editing && syncNotification && createPortal(syncNotification, document.body)}
    ...
    {!session ? <div className="login-layout"><Card className="login-card"><span className="login-emblem"><Icon icon={Key} size={28} /></span><h2>只有主理人，才能编辑这面墙</h2><p>使用 GitHub 仓库授权进入工作台。<br />令牌只留在当前页面，不会存入本机草稿。</p>{!configured && <div className="notice warning"><span>还没有连接作品仓库。请按 README 配置公开的 owner、repo 和 branch，再重新构建网站；此处不接受 GitHub 密码。</span></div>}<form onSubmit={(event) => void connect(event)}><label htmlFor="github-token">GitHub fine-grained PAT</label><Input id="github-token" type="password" value={credential} autoComplete="off" spellCheck={false} placeholder="github_pat_…" required disabled={!configured || busy} onChange={(event) => setCredential(event.target.value)} /><span className="field-help">只授权此仓库的 Contents: Read and write。</span><Button htmlType="submit" type="primary" block loading={busy} disabled={!configured} icon={<Icon icon={Lock} size={16} />}>验证权限，进入工作台</Button></form><a className="text-link" href="https://github.com/settings/personal-access-tokens/new" target="_blank" rel="noreferrer">前往 GitHub 创建限定权限的令牌 ↗</a></Card><aside className="login-note"><Icon icon={Leaf} size={36} /><h3>一件手作，<br />一个值得留下的故事。</h3><ul><li>草稿留在本机，可在下次继续。</li><li>发布后的照片与文字存入 GitHub。</li><li>网站部署完成，其他访客才会看到更新。</li></ul><p>隐藏入口不是权限保护；每次发布都需要 GitHub 的真实写入授权。</p></aside></div> : <>
      ...
      <section className="works-list"><div className="section-heading"><h2>我的作品 <span>{draft?.catalog.products.length || 0}</span></h2><Button size="small" type="primary" disabled={locked || Boolean(editing) || !draft} onClick={(event) => void addProduct(event.currentTarget)} icon={<Icon icon={Plus} size={16} />}>添加新作品</Button></div>{draft?.catalog.products.length ? <div className="works-grid">{draft.catalog.products.map((product) => <Card className="work-row" key={product.id}><div className="work-thumbnail"><PhotoImage photo={product.photos[0]} source={sources[product.photos[0].thumbnail]} /></div><div className="work-row-copy"><h3>{product.name}</h3><span>{product.category || '未分类'} · {product.photos.length} 张照片</span><div><Button size="small" type="text" disabled={locked || Boolean(editing)} onClick={(event) => edit(product, event.currentTarget)}>编辑</Button><Button size="small" type="text" disabled={locked} onClick={() => setPreview(product)}>预览</Button><Button size="small" type="text" danger disabled={locked || Boolean(editing)} onClick={() => setConfirmation({ title: '将这件作品从墙上取下？', description: '先从本地待发布清单移除，点击发布后才会从网站消失。Git 历史仍可能保留旧照片。', action: () => { if (draft) change({ ...draft, catalog: { ...draft.catalog, products: draft.catalog.products.filter((item) => item.id !== product.id) } }); } })}>删除</Button><Button size="small" type="text" className="work-pin-button" disabled={locked || Boolean(editing)} aria-pressed={product.pinOrder !== undefined} title="发布后更新首页置顶顺序" onClick={() => togglePin(product.id)}>{product.pinOrder === undefined ? '置顶' : '取消置顶'}</Button></div></div></Card>)}</div> : <div className="empty-workspace"><Icon icon={Image} size={38} /><h3>从第一件作品开始</h3><p>添加照片，写下它的小故事。<br />准备好了，再一起发布到墙上。</p></div>}<div className="storage-summary"><Icon icon={Folder} size={17} /><span>已加入清单的图片：{formatBytes(totalBytes)}<small>不含本地编辑器和 Git 历史占用</small></span></div><div className="draft-actions"><Button size="small" disabled={!draft || locked} onClick={() => void downloadDraft()}>下载草稿备份</Button><Button size="small" disabled={!draft || locked} aria-describedby="backup-help" onClick={() => backupInputRef.current?.click()}>导入备份</Button><Button size="small" type="text" danger disabled={locked || !draft} onClick={() => setConfirmation({ title: '舍弃本地更改并重新同步？', description: '这会用远端清单替换本机草稿和编辑器。请先下载备份；不会删除远端已发布内容。', action: () => void sync(true) })}>舍弃本地更改</Button></div><input ref={backupInputRef} className="visually-hidden" tabIndex={-1} type="file" accept="application/json,.json" aria-label="选择草稿备份文件" disabled={locked || !draft} onChange={(event) => { void selectBackup(event.target.files?.[0]); event.target.value = ''; }} /><p id="backup-help" className="field-help">JSON 备份（≤300MB）；导入覆盖本机作品集，发布后更新网站。</p></section>
      {editing && <Modal open width={800} className="editor-modal" maskStyle={{ zIndex: 998 }} typewriter={false} onClose={cancelEditor} footer={null} title={<span className="editor-modal-heading"><span>{draft!.catalog.products.some((item) => item.id === editing.id) ? '编辑这份心意' : '记录新的手作'}</span><Button type="text" disabled={locked} aria-label="关闭作品编辑器" onClick={cancelEditor}><Icon icon={X} size={20} /></Button></span>}>
        {syncNotification}
        ...
        <div className="editor-draft-actions"><Button size="small" disabled={locked} onClick={() => void downloadDraft()}>下载草稿备份</Button><Button size="small" disabled={locked} aria-describedby="editor-backup-help" onClick={() => backupInputRef.current?.click()}>导入备份</Button></div><p id="editor-backup-help" className="field-help">JSON 备份（≤300MB）；导入覆盖本机作品集，发布后更新网站。</p>
