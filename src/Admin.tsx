...
import { acknowledgePublication, formatBytes, getFilterOptions, IMAGE_BUDGET, imagePaths, MAX_PHOTOS, messageOf, newProduct, parseCatalog, parsePrice, textLimits, toggleProductPin, usedBytes } from './catalog';
...
export default function Admin() {
  ...
  const priceSnapshot = useRef<{ value: string; error: string } | null>(null);
  const resumeEditor = useRef<HTMLButtonElement>(null);
  ...
  useEffect(() => {
    if (editorHidden) resumeEditor.current?.focus({ preventScroll: true });
    else if (!draft?.editing) editorTrigger.current?.focus({ preventScroll: true });
  }, [Boolean(draft?.editing), editorHidden]);
  ...
  async function connect(event: React.FormEvent) {
    ...
    try {
      ...
      priceSnapshot.current = null; setEditorHidden(false);
  ...
  async function publish() {
    if (!session || !draft || draft.editing || !snapshot || busy) return;
  ...
  function edit(product: Product, trigger: HTMLElement) {
    const saved = currentDraft.current;
    if (!saved || saved.editing || busy) return;
  ...
  function togglePin(id: string) {
    const saved = currentDraft.current;
    if (!saved || saved.editing || busy || file) return;
    try {
      change({ ...saved, catalog: { ...saved.catalog, products: toggleProductPin(saved.catalog.products, id) } });
      setError('');
    } catch (reason) { setError(messageOf(reason)); }
  }

  function cancelEditor() {
    if (!draft?.editing || busy || file || preview || confirmation) return;
    setConfirmation({
      title: '放弃当前编辑？',
      description: '编辑器中的修改将被移除，不影响已存入待发布清单的作品。',
      action: () => {
        if (currentDraft.current) change({ ...currentDraft.current, editing: undefined });
        priceSnapshot.current = null; setEditorHidden(false);
        setError(''); setReplaceId(null);
      },
    });
  }

  function hideEditor() {
    if (!saveError || busy || file || preview || confirmation) return;
    if (priceInput.current) priceSnapshot.current = { value: priceInput.current.value, error: priceInput.current.validationMessage };
  ...
  async function downloadDraft() {
    ...
    if (draft.editing && priceSnapshot.current?.error) { setError('当前价格无效，请继续编辑并修正价格后再下载备份；输入已保留'); return; }
    setBusy(true); setError('');
    try {
  ...
  const editorOpen = Boolean(editing) && !editorHidden;
  ...
  const errorNotice = error && <div className="notice error" role="alert"><Icon icon={Bell} size={19} /><span>{error}</span><Button type="text" size="small" aria-label="关闭提示" onClick={() => setError('')}><Icon icon={X} size={16} /></Button></div>;
  const saveNotice = saveError && <div className="notice error" role="alert"><span>{saveError}</span><Button size="small" disabled={busy || !draft} onClick={() => { if (draft) void persist(draft).catch(() => {}); }}>重试保存</Button>{editorOpen && <Button className="editor-return" size="small" disabled={locked} onClick={hideEditor}>保留输入并返回工作台</Button>}</div>;
  const syncNotification = syncNotice && <div className={`animal-notification-root${editorOpen ? ' editor-notification' : ''}`} role="status" aria-live="polite" aria-atomic="true"><div className="animal-notification-group animal-notification-group-top"><NotificationView key={syncNotice.key} item={syncNotice} onRemove={() => setSyncNotice(null)} /></div></div>;

  return <div className="admin-page">
    {!editorOpen && syncNotification && createPortal(syncNotification, document.body)}
    ...
    {!session ? <div className="login-layout"><Card className="login-card"><span className="login-emblem"><Icon icon={Key} size={28} /></span><h2>只有主理人，才能编辑这面墙</h2><p>使用 GitHub 仓库授权进入工作台。<br />令牌只留在当前页面，不会存入本机草稿。</p>{!configured && <div className="notice warning"><span>还没有连接作品仓库。请按 README 配置公开的 owner、repo 和 branch，再重新构建网站；此处不接受 GitHub 密码。</span></div>}<form onSubmit={(event) => void connect(event)}><label htmlFor="github-token">GitHub fine-grained PAT</label><Input id="github-token" type="password" value={credential} autoComplete="off" spellCheck={false} placeholder="github_pat_…" required disabled={!configured || busy} onChange={(event) => setCredential(event.target.value)} /><span className="field-help">只授权此仓库的 Contents: Read and write。</span><Button htmlType="submit" type="primary" block loading={busy} disabled={!configured} icon={<Icon icon={Lock} size={16} />}>验证权限，进入工作台</Button></form><a className="text-link" href="https://github.com/settings/personal-access-tokens/new" target="_blank" rel="noreferrer">前往 GitHub 创建限定权限的令牌 ↗</a></Card><aside className="login-note"><Icon icon={Leaf} size={36} /><h3>一件手作，<br />一个值得留下的故事。</h3><ul><li>草稿留在本机，可在下次继续。</li><li>发布后的照片与文字存入 GitHub。</li><li>网站部署完成，其他访客才会看到更新。</li></ul><p>隐藏入口不是权限保护；每次发布都需要 GitHub 的真实写入授权。</p></aside></div> : <>
      ...
      {!editorOpen && saveNotice}
      {editing && editorHidden && <div className="notice warning"><span>当前编辑仍在本机，尚未存入待发布清单。请先备份或继续编辑；未保存时不要刷新或关闭页面。</span><Button ref={resumeEditor} className="resume-editor" disabled={locked} onClick={() => setEditorHidden(false)}>继续编辑</Button></div>}
      ...
      <section className="works-list"><div className="section-heading"><h2>我的作品 <span>{draft?.catalog.products.length || 0}</span></h2><Button size="small" type="primary" disabled={locked || Boolean(editing) || !draft} onClick={(event) => void addProduct(event.currentTarget)} icon={<Icon icon={Plus} size={16} />}>添加新作品</Button></div>{draft?.catalog.products.length ? <WorkGrid products={draft.catalog.products} sources={sources} disabled={locked || Boolean(editing) || Boolean(preview) || Boolean(confirmation)} onEdit={edit} onPreview={setPreview} onPin={togglePin} onReorder={products => { const saved = currentDraft.current; if (saved && !saved.editing && !locked) change({ ...saved, catalog: { ...saved.catalog, products } }); }} onDelete={product => setConfirmation({ title: '将这件作品从墙上取下？', description: '先从本地待发布清单移除，点击发布后才会从网站消失。Git 历史仍可能保留旧照片。', action: () => { const saved = currentDraft.current; if (saved) change({ ...saved, catalog: { ...saved.catalog, products: saved.catalog.products.filter(item => item.id !== product.id) } }); } })} /> : <div className="empty-workspace"><Icon icon={Image} size={38} /><h3>从第一件作品开始</h3><p>添加照片，写下它的小故事。<br />准备好了，再一起发布到墙上。</p></div>}<div className="storage-summary"><Icon icon={Folder} size={17} /><span>已加入清单的图片：{formatBytes(totalBytes)}<small>不含本地编辑器和 Git 历史占用</small></span></div><div className="draft-actions"><Button size="small" disabled={!draft || locked} onClick={() => void downloadDraft()}>下载草稿备份</Button><Button size="small" disabled={!draft || locked} aria-describedby="backup-help" onClick={() => backupInputRef.current?.click()}>导入备份</Button><Button size="small" type="text" danger disabled={locked || !draft} onClick={() => setConfirmation({ title: '舍弃本地更改并重新同步？', description: '这会用远端清单替换本机草稿和编辑器。请先下载备份；不会删除远端已发布内容。', action: () => void sync(true) })}>舍弃本地更改</Button></div><input ref={backupInputRef} className="visually-hidden" tabIndex={-1} type="file" accept="application/json,.json" aria-label="选择草稿备份文件" disabled={locked || !draft} onChange={(event) => { void selectBackup(event.target.files?.[0]); event.target.value = ''; }} /><p id="backup-help" className="field-help">JSON 备份（≤300MB）；导入覆盖本机作品集，发布后更新网站。</p></section>
      {editing && editorOpen && <Modal open fitViewport width={800} className="editor-modal" maskStyle={{ zIndex: 998 }} typewriter={false} onClose={cancelEditor} footer={<div className="editor-actions"><Button type="text" disabled={locked} onClick={cancelEditor}>取消编辑</Button><Button form="work-editor" disabled={locked || !editing.photos.length} onClick={() => { if (priceInput.current?.reportValidity()) setPreview({ ...editing, name: editing.name || '未命名作品' }); }}>预览详情</Button><Button form="work-editor" htmlType="submit" type="primary" disabled={locked} icon={<Icon icon={Save} size={16} />}>存入待发布清单</Button></div>} title={<span className="editor-modal-heading"><span>{draft!.catalog.products.some((item) => item.id === editing.id) ? '编辑这份心意' : '记录新的手作'}</span><Button type="text" disabled={locked} aria-label="关闭作品编辑器" onClick={cancelEditor}><Icon icon={X} size={20} /></Button></span>}>
        {syncNotification}
        <div className="editor-draft-toolbar"><span className={`save-status ${persistence}`} role="status"><span />{persistence === 'saved' ? '草稿已保存在本机' : persistence === 'saving' ? '正在保存草稿…' : '草稿尚未保存'}</span><Tag color="app-yellow" variant="soft">本地草稿</Tag></div>
        {(error || saveError) && <div className="editor-notices">{errorNotice}{saveNotice}</div>}
        <form id="work-editor" className="editor-card" key={importVersion} onSubmit={saveEditor}><fieldset disabled={locked}>
        ...
        <div className="photo-field-heading"><label>作品照片 <span className="required">*</span></label><span>{editing.photos.length} / {MAX_PHOTOS}</span></div><p className="field-help">第一张是墙面封面。JPEG / PNG / WebP；超过 2400 万像素会自动等比例缩小，超过 10MB 会自动压缩。</p>
        ...
        {editing.photos.length < MAX_PHOTOS && <button type="button" className="upload-zone" disabled={locked} aria-label="选择一张照片" onClick={() => { setReplaceId(null); inputRef.current?.click(); }} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); if (locked) return; if (event.dataTransfer.files.length !== 1) { setError('请每次添加一张照片，并完成预览和裁剪'); return; } setReplaceId(null); selectFile(event.dataTransfer.files[0]); }}><Icon icon={Upload} size={28} /><span className="upload-zone-label">选择一张照片，或拖到这里</span><span>预览 → 裁剪 → 自动压缩</span></button>}
      </fieldset></form></Modal>}
    ...
    {confirmation && <Modal open className="confirmation-modal" title={confirmation.title} typewriter={false} onClose={() => setConfirmation(null)} footer={<><Button onClick={() => setConfirmation(null)}>再想想</Button><Button type="primary" danger onClick={() => { confirmation.action(); setConfirmation(null); }}>确认</Button></>}><p className="confirmation-copy">{confirmation.description}</p></Modal>}
  </div>;
}
