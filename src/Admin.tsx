...
import { Button, Card, Icon, Input, Modal, Tag } from 'animal-island-ui';
import { acknowledgePublication, formatBytes, IMAGE_BUDGET, imagePaths, MAX_PHOTOS, messageOf, newProduct, parseCatalog, textLimits, usedBytes } from './catalog';
...
export default function Admin() {
  ...
  async function sync(discard = false) {
    ...
    try {
      ...
      if (discard || !saved) {
        ...
        if (saved.baseSha !== remote.sha) setError('远端已有新的提交。本地草稿未被覆盖；请先下载备份，再选择舍弃本地更改并同步，手动重新应用需要的内容');
      }
    } catch (reason) { setError(messageOf(reason)); }
    finally { setBusy(false); }
  }
  ...
  function edit(product: Product) {
    if (!draft) return;
    change({ ...draft, editing: structuredClone(product) });
    setError('');
  ...
  return <div className="admin-page">
    <div className="admin-heading"><div><span className="eyebrow">THE LITTLE WORKSHOP</span><h1>手作工作台<Icon name="Paintbrush" size={27} /></h1><p>把用心完成的小作品，贴到这面墙上。</p></div><a className="back-link" href="#"><Icon name="Leaf" size={16} /> 返回作品墙</a></div>
    {error && <div className="notice error" role="alert"><Icon name="Bell" size={19} /><span>{error}</span><Button type="text" size="small" aria-label="关闭提示" onClick={() => setError('')}><Icon name="Close" size={16} /></Button></div>}
    {!session ? <div className="login-layout"><Card className="login-card"><span className="login-emblem"><Icon name="Key" size={28} /></span><h2>只有主理人，才能编辑这面墙</h2><p>使用 GitHub 仓库授权进入工作台。<br />令牌只留在当前页面，不会存入本机草稿。</p>{!configured && <div className="notice warning"><span>还没有连接作品仓库。请按 README 配置公开的 owner、repo 和 branch，再重新构建网站；此处不接受 GitHub 密码。</span></div>}<form onSubmit={(event) => void connect(event)}><label htmlFor="github-token">GitHub fine-grained PAT</label><Input id="github-token" type="password" value={credential} autoComplete="off" spellCheck={false} placeholder="github_pat_…" required disabled={!configured || busy} onChange={(event) => setCredential(event.target.value)} /><span className="field-help">只授权此仓库的 Contents: Read and write。</span><Button htmlType="submit" type="primary" block loading={busy} disabled={!configured} icon={<Icon name="Lock" size={16} />}>验证权限，进入工作台</Button></form><a className="text-link" href="https://github.com/settings/personal-access-tokens/new" target="_blank" rel="noreferrer">前往 GitHub 创建限定权限的令牌 ↗</a></Card><aside className="login-note"><Icon name="Leaf" size={36} /><h3>一件手作，<br />一个值得留下的故事。</h3><ul><li>草稿留在本机，可在下次继续。</li><li>发布后的照片与文字存入 GitHub。</li><li>网站部署完成，其他访客才会看到更新。</li></ul><p>隐藏入口不是权限保护；每次发布都需要 GitHub 的真实写入授权。</p></aside></div> : <>
      <div className="admin-toolbar"><span className="session-label"><Icon name="User" size={17} /> {session.login}<small>{repository.owner}/{repository.repo} · {repository.branch}</small></span><div><Button size="small" disabled={locked} onClick={() => void sync()}>核对远端</Button><Button size="small" disabled={locked || persistence !== 'saved'} onClick={() => { setSession(null); setCredential(''); setDraft(null); setWatchRevision(''); }}>退出管理</Button></div></div>
      <div className="publish-panel"><div><span className={`save-status ${persistence}`} role="status"><span />{persistence === 'saved' ? '草稿已保存在本机' : persistence === 'saving' ? '正在保存草稿…' : '草稿尚未保存'}</span><p>{progress || (dirty ? '有尚未发布的更改，访客暂时看不到。' : '可以添加新作品，也可以继续打磨已有的故事。')}</p></div><Button type="primary" loading={busy} disabled={!draft || !dirty || Boolean(editing) || conflict || needsSync || persistence !== 'saved' || locked} onClick={() => void publish()} icon={<Icon name="Upload" size={17} />}>发布到作品墙</Button></div>
      ...
      <section id="work-editor" className="editor-section">{editing ? <Card className="editor-card"><div className="section-heading"><h2 ref={editorRef} tabIndex={-1}>{draft!.catalog.products.some((item) => item.id === editing.id) ? '编辑这份心意' : '记录新的手作'}</h2><Tag color="app-yellow" variant="soft">本地草稿</Tag></div><form onSubmit={saveEditor}><fieldset disabled={locked}>
        ...
        <div className="field-grid"><div><label htmlFor="work-category">分类</label><Input id="work-category" maxLength={textLimits.category} value={editing.category} placeholder="例如：编织小物" onChange={(event) => updateEditor({ category: event.target.value })} /></div><div><label htmlFor="work-size">尺寸</label><Input id="work-size" maxLength={textLimits.size} value={editing.size} placeholder="例如：约 12 × 8 cm" onChange={(event) => updateEditor({ size: event.target.value })} /></div></div><label htmlFor="work-material">材质</label><Input id="work-material" maxLength={textLimits.material} value={editing.material} placeholder="例如：棉线、木珠" onChange={(event) => updateEditor({ material: event.target.value })} />
        ...
        {editing.photos.length < MAX_PHOTOS && <div className="upload-zone" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); if (locked) return; if (event.dataTransfer.files.length !== 1) { setError('请每次添加一张照片，并完成预览和裁剪'); return; } setReplaceId(null); selectFile(event.dataTransfer.files[0]); }}><Icon name="Upload" size={28} /><Button type="text" onClick={() => { setReplaceId(null); inputRef.current?.click(); }}>选择一张照片，或拖到这里</Button><span>预览 → 裁剪 → 自动压缩</span></div>}
        <div className="editor-actions"><Button type="text" onClick={() => setConfirmation({ title: '放弃当前编辑？', description: '编辑器中的修改将被移除，不影响已存入待发布清单的作品。', action: () => change({ ...draft!, editing: undefined }) })}>取消编辑</Button><Button disabled={!editing.photos.length} onClick={() => setPreview({ ...editing, name: editing.name || '未命名作品' })}>预览详情</Button><Button htmlType="submit" type="primary" icon={<Icon name="Save" size={16} />}>存入待发布清单</Button></div>
      </fieldset></form></Card> : <div className="editor-placeholder"><Icon name="Paintbrush" size={47} /><h3>给每份手作，留下一点记录。</h3><p>选择左侧作品继续编辑，<br />或从一张新照片开始。</p><span>MAKE SOMETHING THAT MAKES YOU SMILE</span></div>}</section></div>
      <p className="privacy-note"><Icon name="Lock" size={15} /> 只上传可公开的内容。网站发布不是私密存储；删除作品不等于清除 Git 历史或访客已下载的副本。</p>
    </>}
