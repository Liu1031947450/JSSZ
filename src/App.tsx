...
export default function App() {
  ...
  return <div className="site-shell">
    ...
    <header className="site-header"><a className="brand" href="#" aria-label="简时手作首页"><span className="brand-mark"><img src={assetUrl('logo.png')} alt="简时手作 Logo" width={256} height={256} decoding="async" /></span><span>简时手作<small>JIANSHI · HANDMADE</small></span></a><nav aria-label="主导航">{!admin && <><a className="nav-active" href="#works">作品墙<span /></a><a href="#about">关于手作</a></>}<span className="header-note"><Icon name="Sun" size={17} /> 日子慢慢，心意满满</span></nav></header>
    <main id="main" tabIndex={-1}>
      {admin ? <Suspense fallback={<div className="page-status" role="status">正在打开手作工作台…</div>}><Admin /></Suspense> : <>
        <section className="hero" aria-labelledby="site-title"><div className="hero-flower" aria-hidden="true"><Icon name="Flower" size={48} /><span>made with love</span></div><div className="hero-copy"><span className="eyebrow hero-eyebrow"><span /> A LITTLE JOY, MADE BY HAND <span /></span><h1 id="site-title">简时手作<span className="title-spark" aria-hidden="true">✳</span></h1><p>把日子，做成喜欢的样子。</p><span className="hero-subtitle">一些手作 · 一点灵感 · 一份认真生活的心意</span></div><div className="hero-label" aria-hidden="true"><Icon name="Heart" size={22} /><span>慢一点<br />也很好</span><small>JUST TAKE IT SLOW</small></div></section>
        {error && <div className="notice error" role="alert"><Icon name="Wifi" size={19} /><span>{catalog ? '暂时无法获取最新作品，以下为本次已加载的内容。' : '还没能读到作品清单，请稍后再试。'}<small>{error}</small></span><Button size="small" onClick={() => setReload((count) => count + 1)} disabled={loading}>重新加载</Button></div>}
        {loading && !catalog && <div className="page-status" role="status"><Icon name="Leaf" size={25} /><span>正在把小小的心意贴上墙…</span></div>}
        {catalog && <PhotoWall products={catalog.products} />}
        <section className="about-section" id="about" aria-labelledby="about-title"><div className="about-heading"><span className="eyebrow">LITTLE THINGS MATTER</span><h2 id="about-title">手作，是和生活的温柔对话。</h2><p>不追赶时间，不批量复制。<br className="mobile-break" />让平凡的材料，长出自己的小故事。</p></div><div className="about-values"><div><span className="value-icon sage"><Icon name="Leaf" size={25} /></span><h3>自然的灵感</h3><p>从四季与日常里，<br />捡起一点小小的美好。</p></div><div><span className="value-icon peach"><Icon name="Heart" size={25} /></span><h3>手心的温度</h3><p>慢慢打磨每个细节，<br />也留下手作独有的痕迹。</p></div><div><span className="value-icon butter"><Icon name="Gift" size={25} /></span><h3>独一份心意</h3><p>珍藏那些不必完美，<br />却足够真诚的喜欢。</p></div></div></section>
      </>}
    </main>
    <footer className="site-footer"><div className="footer-signature"><Icon name="Leaf" size={17} /><span>慢慢做，好好生活。</span></div><div className="footer-bottom"><small>© {new Date().getFullYear()} 简时手作 · 个人非商业作品展示</small><div><a href={assetUrl('animal-island-ui-LICENSE.txt')} target="_blank" rel="noreferrer">UI: Animal-Island-UI · CC BY-NC 4.0</a><a className="admin-entry" href="#/admin" aria-label="进入管理工作台" title="管理工作台"><Icon name="Leaf" size={16} /></a></div></div></footer>
  </div>;
}
