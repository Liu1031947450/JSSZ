...
1. 新增作品，填写名称；简介、分类、材质和价格可选。价格单位为人民币（元），支持 0 和最多两位小数，不接受负数或非数字；留空表示暂未标价。旧尺寸数据保留用于兼容，不再作为表单或详情字段显示，也不会被转换成价格。
...
公告墙在大于 800px 时每行四件作品，800px 及以下保持每行两件；480px 及以下缩小公告板留白、卡片间距和装饰，并调整字号以适配窄屏。作品详情采用图片在上、资料在下的居中布局，标题、图片说明和底部按钮也居中。详情照片可点击或按 Enter 打开全屏遮罩，按原比例完整展示；关闭按钮、遮罩空白处或 Esc 返回详情，不退出作品详情。

公告墙顶部可选择「全部」、一级分类（作品的分类）和二级分类（作品的材质）。下拉选项来自当前全部作品，去除首尾空白、空值并去重，不预置虚构分类；每个完整分类或材质字段作为一个选项。两个条件可以分别使用或组合筛选，筛选摘要显示当前条件和数量，无匹配时可一键查看全部。选项和排序按作品数据引用使用 `useMemo` 缓存，筛选、打开详情不重复扫描原始数据；收到新的线上清单后自动更新，失效的筛选值回到全部。异常线上数据仍由清单校验拦截，保留已加载内容并显示重试入口。
