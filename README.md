...
公告墙顶部可选择「全部」，或通过「分类」和「材质」筛选作品。下拉选项来自当前全部作品，去除首尾空白、空值并去重，不预置虚构分类；每个完整分类或材质字段作为一个选项。两个条件可以分别使用或组合筛选，筛选摘要显示当前条件和数量，无匹配时可一键查看全部。选项和排序按作品数据引用使用 `useMemo` 缓存，筛选、打开详情不重复扫描原始数据；收到新的线上清单后自动更新，失效的筛选值回到全部。异常线上数据仍由清单校验拦截，保留已加载内容并显示重试入口。
