...
入口脚本尚未加载或加载失败时，原生 HTML 提示和「重新加载」仍可使用，不依赖应用脚本或外部样式；正式域名同时显示对应备用站入口。应用启动后自动替换该提示，并照常显示重要声明。详情照片失败时也提供重新加载，详情与全屏共用重试，保留当前照片、筛选和排序。

## 本地运行

### 新版 3D 首页

直接访问网站默认打开新版艺术展厅；顶部「返回老版首页」可随时切回，老首页顶部「切换新版首页」返回新版，不刷新页面或重复弹出声明。`?home=legacy` 可直接访问老版，原有 `?home=3d` 链接仍访问新版；刷新及浏览器前进、后退保留 URL 对应的版本。`#/admin` 工作台、编辑、备份与发布逻辑不变。
...
Three.js 仅进入新版且存在作品时按需加载；使用缩略图、共享几何体、像素比及渲染面积上限，持续慢帧时降低分辨率。场景离屏、页面隐藏、打开首屏作品详情或手动暂停时停止连续渲染，遵循系统「减少动态效果」。退出新版时清理事件、观察器、动画循环、纹理、材质、几何体和 WebGL 上下文。WebGL 不可用、上下文丢失或场景加载失败时自动显示静态照片，可重试，不影响下方作品浏览。

以支持 WebGL 2 的现代 Chrome、Firefox、Safari、Edge 为目标；60fps 是目标而非对所有设备、刷新率和省电模式的保证，仍需在目标真机测试。构建中的 Three.js 独立包体积提示不影响老首页和工作台的按需加载边界。

构建并启动本地预览后，可用现有 ego-browser 验证无网格、全部作品逐件可达、各展场切换与详情、触屏滑动、四种屏宽、减少动效、上下文释放、单件／空展厅及弱网降级：
...
弱网回归见 `tests/site-fallback.browser.mjs`：`checkStartupFallback(page, origin)` 验证入口脚本与样式失败后的原生提示和刷新；`checkDetailImageRetry(page, origin)` 验证详情与全屏重试、焦点、当前照片及筛选保留。两者支持传入本地生产预览地址，详情测试需清单中至少有一件多图作品；原有 `checkSiteFallback(page)` 使用开发服务验证清单和缩略图重试。
