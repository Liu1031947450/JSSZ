...
删除右侧的 **置顶** 支持多件作品：先置顶的排在首页第一位，后续置顶依次排在置顶队尾；未置顶作品仍按创建时间从新到旧排列。点击 **取消置顶** 恢复普通排序，再次置顶会加入队尾。首页用暖金色图钉徽标标记置顶作品，分类／材质筛选后仍保持该顺序。置顶和取消置顶先保存在本机，需点击 **发布到作品墙** 才会更新首页；编辑、刷新重连及备份导入导出均保留置顶顺序，旧作品默认不置顶。
...
编辑弹窗中的长表单可内部滚动。关闭按钮、遮罩、Esc 和 **取消编辑** 均先确认放弃；选择 **再想想** 保留当前输入，确认后只清除本次编辑及不再引用的临时图片，不影响待发布清单。裁剪、预览及确认窗口关闭后返回编辑弹窗，不重置未保存字段。弹窗内显示草稿保存状态、错误和重试入口，并可下载或导入备份；刷新后重新登录，或导入含未完成编辑的备份时，会自动恢复编辑弹窗。
...
公告墙筛选的「分类」「材质」标签固定在各自下拉框左侧，窄屏两项分行排列，保持下拉框可用宽度。

公告墙顶部可选择「全部」，或通过「分类」和「材质」筛选作品。下拉选项来自当前全部作品，去除首尾空白、空值并去重，不预置虚构分类；每个完整分类或材质字段作为一个选项。两个条件可以分别使用或组合筛选，筛选摘要显示当前条件和数量，无匹配时可一键查看全部。材质后提供「排序」，可选「价格从低到高」或「价格从高到低」；初始保留默认顺序，置顶作品始终按置顶先后排列，其余作品按所选价格顺序展示，0 元正常参与排序，未标价始终放在普通作品末尾，同价按创建时间从新到旧排列。点击「全部」同时清除筛选并恢复默认排序，打开详情不重置排序，刷新页面回到默认状态。选项和排序按作品数据引用及排序选项使用 `useMemo` 缓存，筛选、打开详情不重复扫描原始数据；收到新的线上清单后自动更新，失效的筛选值回到全部。异常线上数据仍由清单校验拦截，保留已加载内容并显示重试入口。
...
const { checkCatalogEditor, checkProductPins, checkWorkspaceModal, checkBackupImport, checkNewWorkSync } = await import(pathToFileURL('$PWD/tests/catalog-editor.browser.mjs').href);
