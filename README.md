...
2. 点击整个上传区域（含空白、图标和文字）选择照片，或直接拖入照片；键盘聚焦后可按 Enter／空格选择。每次添加一张 JPEG、PNG 或 WebP，每件 1～5 张，原图不超过 2400 万像素。10MB 以内直接打开；超过 10MB 会在本机自动压缩至限制以内，优先保留分辨率、从最高编码质量逐步调整，仍超限才缩小尺寸，尽量保留清晰度。裁剪窗口显示压缩前后大小，失败时可取消并重新选图；不会上传原图。
